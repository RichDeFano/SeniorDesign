...
void Display_obj::pullCSHigh(){
  digitalWrite(CS,HIGH);
  digitalWrite(12,LOW);
  //Serial.println("Disp CS is High.");
  delay(10);
}
...
/*
  ...
  Function Name: removeText
  Funciton Parameters: (int) xPos - The x position of the(start) of the text
                      (int yPos) - The y position of the (start) of the text
                      (int stringSize) - The size of the inputted string
  Function Description: Draw Each frame of a full bitmap with a certain delay.
  */
void Display_obj::removeText(double xPos, double yPos,int stringSize, uint8_t asciiBuff)
{
 bitmapLetter fnt_controller(0,0,0x00,0,0);
bitmapLetter sheldon_alph[0x5A];
fnt_controller.createDictionary(sheldon_alph,6,8);
